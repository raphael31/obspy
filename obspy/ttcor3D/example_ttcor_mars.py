...
using Mars models (Gudkova) and pyMoho models of Mars
...
# set mantle density
rho_m = 3368.0
# set crust density from pyMoho density model assuming 11% porosity
rho_c = 2850.0
# set quake depth
...
# Assume average radius (value consistent with Gudkova model value
topo_r0=float(3389.50)
# load seismoloigal model (Gudkova model + small inner core to be managed by obspy taup) 
thickref=float(50.0)
model_nd='./example_Mars_data/model_Gudkova.nd'
...
#####
## Use of ttcor with
## 1- topography and Moho models from pyMoho (geodetic model)
...
# Impact on Elysium Mons
quake_lat=float(25.02)
quake_lon=float(147.21)
# INSIGHT/ELYS station location (before landing)
stat_lat=float(4.43)
stat_lon=float(135.84)
# load topography model
# import Mars topography (file available in pyMoho)
degmax = 360
topofile = './example_Mars_data/MarsTopo719.shape'
topo = pyshtools.SHCoeffs.from_file(topofile, degmax)
# load moho shape model (computed by pyMoho for an average crustal thickness of 50 km and rho_c, rho_m)
moho_pad = pyshtools.SHCoeffs.from_file('./example_Mars_data/mohosave'+str(int(thickref))+'.sh', topo.lmax)
# load seismoloigal model (VPREMOON + small inner core to be managed by obspy taup) 
thickref=float(28.0)
model_nd='./example_Mars_data/model_Gudkova.nd'
# load taup tables
model_npz='./example_Mars_data/model_Gudkova.npz'
# AND, if not already done, compute (may be long computation) with
#obspy.taup.taup_create.build_taup_model(model_nd,'./example_Mars_data')
...
# compute P and S corrections at ELYS station
...
# compute P and S corrections and travel times at ELYS station
